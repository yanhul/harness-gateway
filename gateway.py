...
STATES = {"OBSERVED","DIAGNOSED","AWAITING_HUMAN","AUTHORIZED","ACTING","VERIFYING","PERSISTED","RESUMED","REJECTED","EXPIRED","REVOKED","BLOCKED"}
ALLOWED_TRANSITIONS = {
...
class Ledger:
    ...
    def create(self, p: dict[str,Any], ttl: int = 900):
        repo, wf, run_id, sha = p["repo"], p["workflow"], p["run_id"], p["head_sha"]
        if repo not in REPOS or not isinstance(run_id,int) or run_id <= 0 or not SHA_RE.fullmatch(sha):
            raise ValueError("invalid webhook identity")
        if p.get("conclusion") not in ("failure","timed_out","cancelled"):
        ...
        gov = p.get("governance_digest") or digest("UNBOUND-GOVERNANCE-DIGEST")
        t = now(); exp = t + ttl
        self.db.execute("INSERT INTO tickets VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (ticket,key,repo,wf,run_id,sha,p.get("failure","ci_failure"),p.get("diagnosis",""),
             p.get("proposed_action","repair implementation"),gov,digest(nonce),"OBSERVED",t,exp,0))
        self.db.execute("UPDATE tickets SET status='DIAGNOSED' WHERE ticket=?",(ticket,))
        self.db.execute("UPDATE tickets SET status='AWAITING_HUMAN' WHERE ticket=?",(ticket,))
        self._event(ticket,"TICKET_CREATED","github-webhook",{"event_key":key,"repo":repo,"run_id":run_id,"head_sha":sha})
        self.db.commit()
    ...
    def command(self, sender: str, text: str, gateway_token_ok: bool, governance_digest: str|None=None):
        ...
        cmd=parts[0].upper()
        if cmd == "STATUS":
            return [dict(r) for r in self.db.execute("SELECT ticket,repo,head_sha,status,expires_at FROM tickets ORDER BY created_at DESC LIMIT 20")]
        if cmd == "STOP":
        ...
        nonce=parts[2]
        if not hmac.compare_digest(digest(nonce),row["nonce_hash"]): raise PermissionError("bad nonce")
        ...
        self._transition(ticket,"AUTHORIZED",sender)
        self.db.commit(); return {"ticket":ticket,"status":"AUTHORIZED","repo":row["repo"],"head_sha":row["head_sha"],"governance_digest":row["governance_digest"]}
    ...
    def do_POST(self):
        ...
        try:
            if self.path == "/github/webhook":
                if not verify_github_signature(body,self.headers.get("X-Hub-Signature-256",""),os.environ["GITHUB_WEBHOOK_SECRET"]): return self._json(401,{"error":"bad_signature"})
                p=json.loads(body); r=self.ledger.create(p)
