...
def normalize_workflow_run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if run_url != expected_url: raise ValueError("invalid run_url")
    ...
    return {"repo":full_name,"workflow":workflow,"run_id":run_id,"head_sha":sha,"conclusion":conclusion,"run_url":run_url,"governance_digest":gov,"failure":payload.get("failure","ci_failure"),"diagnosis":payload.get("diagnosis",""),"proposed_action":payload.get("proposed_action","repair implementation")}
...
class Ledger:
    def __init__(self, path: str = "gateway.db"):
        ...
        """)
        self._ensure_auth_schema(); self.db.commit()
    def _ensure_auth_schema(self):
        cols={r[1] for r in self.db.execute("PRAGMA table_info(repair_authorizations)").fetchall()}
        if "revoked" not in cols: self.db.execute("ALTER TABLE repair_authorizations ADD COLUMN revoked INTEGER NOT NULL DEFAULT 0")
    def _event(self,ticket,event,actor,payload):
        row=self.db.execute("SELECT event_hash FROM events ORDER BY id DESC LIMIT 1").fetchone(); prev=row[0] if row else "0"*64
        body=json.dumps(payload,sort_keys=True,separators=(",",":")); eh=digest(prev+event+actor+body)
        self.db.execute("INSERT INTO events(ticket,event,actor,payload,created_at,prev_hash,event_hash) VALUES(?,?,?,?,?,?,?)",(ticket,event,actor,body,now(),prev,eh))
    def verify_event_chain(self):
        prev="0"*64
        for r in self.db.execute("SELECT id,ticket,event,actor,payload,prev_hash,event_hash FROM events ORDER BY id").fetchall():
    ...
    def create(self,p,ttl=900):
        ...
        key=f"{repo}|{wf}|{run_id}|{sha}"; existing=self.db.execute("SELECT * FROM tickets WHERE event_key=?",(key,)).fetchone()
        ...
        self.db.execute("INSERT INTO tickets VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(ticket,key,repo,wf,run_id,sha,p.get("failure","ci_failure"),p.get("diagnosis",""),p.get("proposed_action","repair implementation"),gov,digest(nonce),"OBSERVED",t,exp,0))
        self._event(ticket,"TICKET_CREATED","github-webhook",{"event_key":key,"repo":repo,"workflow":wf,"run_id":run_id,"head_sha":sha}); self._transition(ticket,"DIAGNOSED","gateway"); self._transition(ticket,"AWAITING_HUMAN","gateway"); self.db.commit()
        return dict(self.db.execute("SELECT * FROM tickets WHERE ticket=?",(ticket,)).fetchone()),nonce
    def get(self,ticket):
        r=self.db.execute("SELECT * FROM tickets WHERE ticket=?",(ticket,)).fetchone(); return dict(r) if r else None
    def command(self,sender,text,gateway_token_ok,governance_digest=None):
        ...
        if cmd=="STOP":
            ...
            for r in rows:
                self.db.execute("UPDATE repair_authorizations SET revoked=1 WHERE ticket=? AND consumed=0",(r[0],)); self._transition(r[0],"REVOKED",sender)
        ...
        if cmd=="BOQUA": self._transition(ticket,"REJECTED",sender); self.db.commit(); return {"ticket":ticket,"status":"REJECTED"}
        if cmd=="RETRY":
        ...
        if governance_digest is not None and governance_digest != row["governance_digest"]:
            self._transition(ticket,"BLOCKED",sender); self.db.commit(); raise PermissionError("governance digest mismatch")
        self.db.execute("UPDATE tickets SET nonce_used=1 WHERE ticket=?",(ticket,)); self._transition(ticket,"AUTHORIZED",sender); self.db.commit()
    ...
    def repair_token(self,ticket,repo,sha,gov):
        ...
        self.db.execute("INSERT INTO repair_authorizations VALUES(?,?,?,?,?,?,?,?,?)",(digest(raw),ticket,repo,sha,gov,t,exp,0,0)); self._transition(ticket,"ACTING","repair-worker"); self._event(ticket,"REPAIR_AUTH_ISSUED","gateway",{"repo":repo,"head_sha":sha,"expires_at":exp}); self.db.commit()
    ...
    def consume_repair_token(self,raw,ticket,repo,sha,gov):
        ...
        token_hash=digest(raw); row=self.db.execute("SELECT * FROM repair_authorizations WHERE token_hash=?",(token_hash,)).fetchone()
        ...
        self._event(ticket,"REPAIR_AUTH_CONSUMED","repair-worker",{"repo":repo,"head_sha":sha}); self.db.commit(); return {"ticket":ticket,"repo":repo,"head_sha":sha,"governance_digest":gov,"effect":"repair"}

def verify_github_signature(body: bytes, signature: str, secret: str) -> bool:
    if not signature.startswith("sha256="): return False
    expected="sha256="+hmac.new(secret.encode(),body,hashlib.sha256).hexdigest(); return hmac.compare_digest(expected,signature)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if n<0 or n>MAX_BODY_BYTES: return self._json(413,{"error":"request_too_large"})
        ...
        try:
            ...
            if self.path=="/sms/command":
                ...
                p=json.loads(body)
                out=self.ledger.command(self.headers.get("X-SMS-Sender",""),p["text"],ok)
            ...
            if self.path=="/repair/authorize":
                ...
                if raw: return self._json(200,self.ledger.consume_repair_token(raw,p["ticket"],p["repo"],p["head_sha"],p["governance_digest"]))
