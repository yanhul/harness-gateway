...
class GatewayTests(unittest.TestCase):
    ...
    def test_receipt_requires_exact_contract_binding_and_fence(self):
        ...
        for field in ("effect_id", "idempotency_key", "evidence_ref", "lineage_ref", "attempt_fence"):
            ...
            with self.subTest(field=field):
                with self.assertRaises(GatewayError): self.g.record_receipt("T1", bad)
    ...
    def test_stale_worker_receipt_is_recorded_but_cannot_mutate_new_attempt(self):
        self._authorize(); first = self._begin()
        self.g.reconcile("T1", first["attempt_id"], "+84999", "NO_EFFECT", "verify:crash-no-effect")
        self.g.authorize_retry("T1", "+84999")
        second = self._begin()
        self.g.record_receipt("T1", self._receipt(first, status="COMPLETED"))
    ...
    def test_receipt_history_survives_restart(self):
        ...
        self.g.authorize_retry("T1", "+84999"); second = self._begin()
    ...
    def test_cross_process_attempt_allocation_is_serialized(self):
        ...
        self.assertEqual([r[1]["attempt_id"] for r in successes], ["T1:1"])
    ...
    def test_cross_process_receipt_race_has_one_winner(self):
        self._authorize(); effect = self._begin(); receipt = self._receipt(effect)
        ctx = multiprocessing.get_context("spawn"); queue = ctx.Queue()
        ...
        results = [queue.get(timeout=5) for _ in processes]
        successes = [r for r in results if r[0] == "ok"]; failures = [r for r in results if r[0] == "error"]
        self.assertEqual(len(successes), 1, results); self.assertEqual(len(failures), 1, results)
