...
function makeDO(env={}) {
  const state = { storage: { sql: new SqlAdapter() }, async blockConcurrencyWhile(fn) { return fn(); } };
  return new GatewayDO(state, {
    GITHUB_WEBHOOK_SECRET: 'github-secret', REPO_ALLOWLIST: 'yanhul/harness-gateway',
    SMS_BRIDGE_TOKEN: 'sms-secret', AUTHORIZED_SMS_SENDERS: '+84123456789',
    REPAIR_WORKER_TOKEN: 'worker-secret', ...env,
  });
}
...
function workerReq(path, body) {
  return new Request('https://gateway'+path, {method:'POST',headers:{'X-Repair-Worker-Token':'worker-secret','content-type':'application/json'},body:JSON.stringify(body)});
}
...
test('runtime: webhook -> SUA -> worker auth -> consume -> full lifecycle -> audit', async () => {
  ...
  const wr=event();
  const first=await signedWebhook(gw,wr); assert.equal(first.status,201);
  const created=await first.json(); assert.equal(created.status,'AWAITING_HUMAN');
  const sua=await sms(gw,`SUA ${created.ticket} ${created.nonce}`); assert.equal(sua.status,200);
  const authorized=await sua.json(); assert.equal(authorized.status,'AUTHORIZED');
  ...
  const issue=await gw.issue(workerReq('/repair/token',binding)); assert.equal(issue.status,200);
  ...
  for (const status of ['VERIFYING','PERSISTED','RESUMED']) {
    const r=await gw.lifecycle(workerReq('/repair/lifecycle',{...consumeBody,status}));
    assert.equal(r.status,200);
  }
  assert.deepEqual(await (await gw.audit(workerReq('/audit/verify'))).json(),{valid:true});
});

test('runtime: adversarial boundaries reject spoofing, sender mismatch, binding mismatch and replay', async () => {
  const gw=makeDO(); await gw.init(); const wr=event(123457);
  ...
  const binding={ticket:created.ticket,repo:wr.repository.full_name,head_sha:wr.workflow_run.head_sha,governance_digest:wr.governance_digest};
  const issue=await gw.issue(workerReq('/repair/token',binding)); assert.equal(issue.status,200); const issued=await issue.json();
...
test('runtime: duplicate webhook is idempotent and state remains closed', async () => {
  const gw=makeDO(); await gw.init(); const wr=event(123458);
  const a=await signedWebhook(gw,wr); const aj=await a.json();
  const b=await signedWebhook(gw,wr); const bj=await b.json();
  assert.equal(a.status,201); assert.equal(b.status,200); assert.equal(bj.duplicate,true);
  assert.equal(bj.ticket,aj.ticket); assert.equal(bj.status,'AWAITING_HUMAN'); assert.equal(await gw.verifyChain(),true);
});
